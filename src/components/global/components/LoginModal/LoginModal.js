...
const LoginModal = () => {
    ...
    const CheckEmail = async () =>{    
        ...
        const data = await fetchData('http://grub24.com/NextApi/CheckEmail?debug=2','POST',formData)
        console.log(data)
        setLoading(false)
        if(data.status === 200){
            setDetail({
                ...detail,
                need_verify:data.result.need_verify,
                phone_verify:data.result.phone_verify,
                verify_token:data.result.verify_token,
                is_register:data.result.is_register
            })
        }else{
    ...
    const LoginUser = async () => {
        ...
        const formData = new FormData();
        formData.append('email',detail.email);
        formData.append('pass',detail.pass);
        formData.append('deviceId',getDeviceId())
        const data = await fetchData('http://grub24.com/NextApi/Login','POST',formData);
        if(data.success){
            console.log(data)
        ...
        // await axios.post(`http://grub24.com/NextApi/Login`,formData,{withCredentials:true}).then(res=>{
  ...
  return (
    <>
    {
        ...
        <div className={`${styles.childComp} ${loginModal && styles.openLogin}` }>
            {
                detail.need_verify && !detail.is_register ?
                ...
                <>
                    ...
                    <div className={styles.botComp}>
                        ...
                        <button 
                        onClick={CheckEmail}
                        className={`roboto400 ${styles.btEmail}`}>
                            <span>
                ...
                :  !detail.is_register && !detail.need_verify?
